...
import PatientEhr from "./PatientNotes";
...
export default function VideoConsultation() {
  ...
  const systemEndingRef = useRef(false);
  ...
  const uid = doctor?.id ? Number(doctor.id) : null;
  ...
  useEffect(() => {
    ...
    const updateTimer = async () => {
      ...
      if (remaining <= 0 && !autoEndedRef.current) {
        ...
        systemEndingRef.current = true;

        try {
          let response = null;

          if (videoCall?.id) {
            response = await endCall(videoCall.id, token);
          }
        } catch (e) {
        ...
        Swal.fire({
          ...
          title: "Consultation Ended",
          text: response?.message || "The consultation has ended.",
        });

        navigate(doctor ? "/doctor/appointments" : "/patient/appointments");
      }
    };
      ...
      >
        ...
        {!remoteJoined && (
          ...
          >
            ...
            >
              The consultation will begin automatically once the patient joins.
            </p>
          </div>
        )}
        ...
        >
          ...
          >
            {remoteJoined ? "Patient Connected" : "Waiting for Patient"}
          </div>
        </div>
