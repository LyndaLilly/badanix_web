...
export default function VerifyResetCode() {
  ...
  if (name.length <= 2) {
    ...
    maskedName =
      name.substring(0, 2) +
      "*".repeat(Math.max(name.length - 2, 3));
  }
  ...
  const maskedDomain =
    domainName[0] +
    "*".repeat(Math.max(domainName.length - 1, 2));
  ...
  const verifyCode = async (e) => {
    ...
    try {
      const response = await fetch(
        ApiUrl.VERIFY_PASSWORD_CODE_PATIENT,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        navigate("/patient/resetpassword");
  ...
  const resendCode = async () => {
    ...
    try {
      const response = await fetch(
        ApiUrl.RESEND_PASSWORD_CODE_PATIENT,
        {
          method: "POST",
          headers: {
            Accept: "application/json",
